...
const fetchBlogs = async () => {
  const blogData = await fetch('http://localhost:8080/memoList.php');
...
const blogOutput = async () => {
  ...
  if (blogs.length === 0)
    return (memoList.innerHTML = '<p>何も入力されていません</p>');
  blogs.map(({ title, content }) => {
    blogList.insertAdjacentHTML(
      'beforeend',
      `<div class='blog_content'><p class='mb-0 font-weight'>タイトル</p><p class='mt-0'>${title}</p><p class='mb-0 font-weight'>内容</p><p class='mt-0'>${content}</p></div>`
    );
  });
};
